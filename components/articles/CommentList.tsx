...
function CommentList({ articleId, productId }: Props) {
  ...
  const { mutate: patchComment, isPending: isPendingPatch } = useMutation({
    mutationFn: (data: { commentId: string; content: string }) =>
      api.editComment(data.commentId, data.content),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['comments', articleId ? { articleId } : { productId }],
      });
  ...
  const handleRegistEditClick = (commentId: string, editedContent: string) => {
  ...
  const comments: CommentItemDto[] = data?.comments || [];
